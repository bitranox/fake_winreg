...
function cleanup() {
  ...
  rm -f  /usr/local/bin/fake_winreg
  cd "${save_dir}" || exit
...
function test_commandline_interface_venv() {
  ...
  clr_green "issuing command : /usr/local/bin/fake_winreg --version"
  if ! "/usr/local/bin/fake_winreg" --version; then
...
function test_commandline_interface_venv_old() {
  ...
  clr_green "issuing command : $HOME/venv/local/bin/fake_winreg --version"
  if ! "$HOME/venv/local/bin/fake_winreg" --version; then
    my_banner_warning "test commandline interface on virtual environment ERROR"
    beep
    sleep "${sleeptime_on_error}"
    return 1
