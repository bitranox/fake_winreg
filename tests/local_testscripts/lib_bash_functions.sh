...
function run_pytest() {
  ...
  if ! python3 -m pytest "${project_root_dir}" "$@" --cov=fake_winreg; then
...
function test_commandline_interface_venv() {
  ...
  clr_green "issuing command : $HOME/venv/bin/fake_winreg --version"
  if ! "$HOME/venv/bin/fake_winreg" --version; then
