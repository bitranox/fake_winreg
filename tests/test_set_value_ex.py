...
@pytest.fixture(params=[None, 1, 'test', ['a', 'b'], ['a', 1], (chr(128512) * 10).encode('utf-8')],
                ids=['none', 'int', 'str', 'multi_str', 'multi_str_malformed', 'bytes'])
def data(request: Any) -> Any:
    return request.param


def test_write_value(key_handle_test_all_access, reg_type, data):
